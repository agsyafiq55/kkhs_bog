...
<x-layouts.app>
    ...
    <!-- Grid to display events in rows of 4 cards -->
                    ...
                    @foreach($events as $event)
                        <article class="group flex rounded-radius max-w-sm flex-col overflow-hidden border border-outline bg-surface-alt dark:border-outline-dark dark:bg-surface-dark-alt">
                            <div class="h-44 md:h-64 overflow-hidden">
                                <img src="data:image/jpeg;base64,{{ base64_encode($event->thumbnail) }}" class="object-cover transition duration-700 ease-out group-hover:scale-105" alt="{{ $event->title }}" />
                            </div>
