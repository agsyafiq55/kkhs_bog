<div class="py-6">
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <flux:heading size="xl" level="1" class="text-gray-800 dark:text-white">{{ __('Existing Events') }}</flux:heading>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Manage and organize your school events</p>
        </div>
        <flux:button href="{{ route('admin.events.create') }}" class="bg-indigo-600 hover:bg-indigo-700 transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($events as $event)
            <div class="group relative bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-700"
                ...
                <!-- Event Content -->
                <div class="p-5">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2 line-clamp-1">{{ $event->title }}</h3>
