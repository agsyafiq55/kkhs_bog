...
<div class="py-6">
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ __('Create, View, Update or Delete Timeline Cards.') }}
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-zinc-700 mb-6">
                <flux:heading size="lg" class="mb-4">{{ $editingCardId ? 'Edit Timeline Card' : 'Add New Timeline Card' }}</flux:heading>
                    ...
                    <div class="space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <flux:select wire:model="side" class="w-full">
                        ...
                        <div>
                            ...
                            <flux:input type="text" wire:model="title" class="w-full" placeholder="Enter title" />
                            @error('title') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                        </div>
                        
                        <div>
                            <flux:text variant="strong" class="mb-2 block text-gray-700 dark:text-gray-300">Description (English)</flux:text>
                            <flux:textarea wire:model="description" class="w-full" rows="3" placeholder="Enter description in English"></flux:textarea>
                            @error('description') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                        </div>
                        
                        <div>
                            <flux:text variant="strong" class="mb-2 block text-gray-700 dark:text-gray-300">Description (Chinese)</flux:text>
                            <flux:textarea wire:model="description_zh" class="w-full" rows="3" placeholder="Enter description in Chinese"></flux:textarea>
                            @error('description_zh') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-zinc-700 mb-6">
                ...
                <div class="space-y-3">
                    <flux:button type="button" wire:click="saveCard" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="saveCard">{{ $editingCardId ? 'Update Card' : 'Add Card' }}</span>
        ...
        <div x-data="{
            ...
            <ul x-ref="cardsList" class="space-y-4">
                @foreach($cards as $card)
                    <li class="bg-gray-50 dark:bg-zinc-800 p-5 rounded-lg shadow-sm border border-gray-100 dark:border-zinc-700 flex justify-between items-center" data-id="{{ $card->id }}">
                        <div>
                            <div class="flex items-center space-x-3 mb-2">
                                <span class="font-bold text-lg text-gray-800 dark:text-white">{{ $card->year }}</span>
                                <span class="px-2 py-1 rounded-full text-xs {{ $card->side === 'left' ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300' : 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300' }}">
                                    {{ ucfirst($card->side) }}
                                </span>
                            </div>
                            <h3 class="font-medium text-gray-800 dark:text-white">{{ $card->title }}</h3>
                            @if($card->description)
                        ...
                        <div class="flex space-x-2">
                            <button wire:click="editCard({{ $card->id }})" class="p-2 text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                </svg>
                            </button>
                            <button wire:click="deleteCard({{ $card->id }})" class="p-2 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 transition-colors" onclick="confirm('Are you sure you want to delete this card?') || event.stopImmediatePropagation()">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
